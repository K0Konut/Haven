...
// On remplace l'import SMS par EmailJS
...
export default function SettingsScreen() {
  ...
  const [contact, setContact] = useState<EmergencyContact>({
    email: "", // Remplacé phone par email
    message: DEFAULT_MSG,
  });
  const [status, setStatus] = useState<string | null>(null);
  ...
  async function handleTestEmail() {
    ...
    const templateParams = {
      to_email: email,
      message: (contact.message?.trim() || DEFAULT_MSG) + " (Ceci est un TEST)",
      location: fix ? `${fix.lat}, ${fix.lng}` : "Non disponible"
    };

    try {
      // Note : Remplacez les 'YOUR_SERVICE_ID' etc. par vos vrais IDs EmailJS si nécessaire
      await emailjs.send(
        'YOUR_SERVICE_ID', 
        'YOUR_TEMPLATE_ID', 
        templateParams, 
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4 space-y-3">
        ...
        <p className="text-xs text-zinc-500">
          L’alerte envoie un mail automatique à ton contact avec ta position GPS.
        </p>
